...
## Phase 6 — ارزیابی استحکام (Robustness) ✅ **[بسته شد]**
- [x] 6.1 پایپ‌لاین ۸ نوع corruption — تست کمّی + بصری روی MRI واقعی
- [x] 6.2 اجرای واقعی هر دو مدل روی ۹ حالت (۳۰۸ تصویر test) — کامل شد
- [x] 6.3 تحلیل افت عملکرد — یافته‌ی کلیدی: **«دیوار نویز»** — هر دو مدل با Gaussian/Salt-Pepper
      Noise افت ۸۰-۱۰۰٪ دارند (چون augmentation هرگز نویز نداشت)؛ YOLO11 حتی در بدترین حالت کمی
      عملکرد حفظ می‌کند، Faster R-CNN کاملاً از کار می‌افتد (F1≈0)
- [x] 6.4 گزارش نهایی → `reports/phase6_robustness_report.md`
      + نمودارها: `phase6_degradation_comparison.png`, `phase6_degradation_percent.png`,
        `corruption_types_preview.png` (واقعی)
      + داده‌ی خام: `outputs_v3/logs/robustness/*.json`, `outputs/yolo11/run1/robustness/*.json`
      + `configs/project_config.yaml -> phase6_robustness`
**🚪 گیت ۶: ✅ تایید شد**

## ✅ نقص Phase 5 برطرف شد (اصلاح 2026-08-20)
GPU memory آموزش YOLO11 با روش درست (آموزش واقعی ۱-epoch + خواندن GPU_mem واقعی از لاگ
Ultralytics، نه شبیه‌سازی دستی) به‌دست آمد: **1,218.56 MB**. نسبت با Faster R-CNN (13,444.64 MB)
= ۱۱ برابر — سازگار با نسبت inference (~۹.۵ برابر). گیت ۵ اکنون کاملاً و صادقانه بسته است.

## Phase 7 — تحلیل تبیین‌پذیری (Grad-CAM) 🔄 **[فاز فعلی]**
- [x] 7.1 پیاده‌سازی EigenCAM دستی (نه وابسته به API کتابخانه برای detection که ریسک ناسازگاری دارد)
      — دو باگ واقعی در تست پیدا و اصلاح شد: (۱) نبود ReLU باعث نویز از مقادیر منفی می‌شد،
      (۲) **ابهام علامت SVD** (بردار تکین می‌تواند دقیقاً برعکس نواحی مهم را نشان دهد) اصلاح شد.
      تست نهایی: هم مکان‌یابی جسم ساختگی هم دنبال‌کردن جابه‌جایی آن با تمایز ۴x تایید شد.
- [x] انتخاب نمونه‌های good/bad/failure از داده‌ی واقعی Phase 5 (نه دستی/حدسی) — شامل یک نمونه‌ی
      بسیار آموزنده: تصویری که Faster R-CNN کاملاً ناموفق ولی YOLO11 بسیار موفق بوده (IoU=0.953)
- [x] `run_gradcam_faster_rcnn.py` (لایه‌ی هدف: `backbone.body.layer4`) و
      `run_gradcam_yolo.py` (لایه‌ی هدف: `model.model[9]`=SPPF، تایید شد با بررسی مستقیم معماری) —
      هر دو با تصویر MRI واقعی و وزن پایه end-to-end تست شدند، بدون خطا
- [ ] 7.2 اجرای واقعی روی Kaggle با چک‌پوینت‌های نهایی — **در انتظار اجرای شما**
- [ ] 7.3 تحلیل کیفی (کجا را نگاه می‌کند؟)
- [ ] 7.4 گزارش نهایی
**🚪 گیت ۷: هنوز باز**
